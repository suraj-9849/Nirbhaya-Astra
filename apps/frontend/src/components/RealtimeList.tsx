...
import { cleanText, fetchCityName } from "@/app/lib/utils";
...
export default function RealtimeList() {
  ...
  // Modified section of your RealtimeList component
  React.useEffect(() => {
      ...
      try {
        ...
        const enrichedData = await Promise.all(
          result.map(async (post) => {
            // Only try to get location if coordinates exist
            if (post.Location) {
              const clenLoc = cleanText(post.Location);
              const coordinates = clenLoc.split(",").map(Number);
              if (
